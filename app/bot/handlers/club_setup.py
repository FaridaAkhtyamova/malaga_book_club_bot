...
from app.bot.club_publish import publish_meeting_poll, publish_vote_polls
...
from app.services.meeting_poll import get_selected_book_title, meeting_poll_options
...
async def cmd_start_vote(
    ...
    try:
        cycle, chunks = await CycleService(session).prepare_vote()
    ...
    await publish_vote_polls(bot, dest, cycle, chunks)
    same_thread = (
...
async def cmd_start_meeting_poll(
    ...
    title = get_selected_book_title()
    await publish_meeting_poll(bot, dest, title, meeting_poll_options())
    same_thread = (
        message.chat.id == dest.chat_id and message.message_thread_id == dest.message_thread_id
    )
    if not same_thread:
