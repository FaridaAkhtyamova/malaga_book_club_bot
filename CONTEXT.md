...
- `/create_meeting` uses the poll-chosen meeting date when it exists; otherwise the admin types the date (`25.09` / `25.09.2026`). If there is no winner book, the admin types the title before the time. The group gets a message with **📅 Добавить в календарь** (Safari opens `text/calendar` so iPhone shows Add Event) plus `event.ics` for other clients. `PUBLIC_BASE_URL` serves our file; if empty, the button uses a hosted ICS link.
