...
- `/create_meeting` uses the poll-chosen meeting date when it exists; otherwise the admin types the date (`25.09` / `25.09.2026`). If there is no winner book, the admin types the title before the time. The group gets `event.ics` (simple VEVENT, `TZID=Europe/Madrid`, MIME `text/calendar`). On iPhone tap the file to open the system Add Event sheet. If `PUBLIC_BASE_URL` is set, a **📅 Добавить в календарь** button serves the same file from `/invite/{token}.ics`.
...
- DM commands use the user's active bound group; if they belong to several, the bot asks which group to use in DM (`/clubs` is not in the member menu)
