...
def test_ics_uses_madrid_tzid_and_omits_publish_method() -> None:
...
def test_ics_upload_uses_text_calendar_mime() -> None:
    ...
    assert document_upload_fields("knizhny-klub.ics") == {
        "filename": "knizhny-klub.ics",
...
async def test_ics_http_serves_text_calendar(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from app.services.meeting_invite import ICS_CONTENT_TYPE, ICS_FILENAME
    ...
    async with TestClient(TestServer(app)) as client:
        ...
        assert ICS_FILENAME in response.headers["Content-Disposition"]
